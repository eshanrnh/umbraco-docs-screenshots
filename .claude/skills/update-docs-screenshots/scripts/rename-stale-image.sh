...
#
# Strips a trailing version marker (any digits, as a `-v9`/`_v9`/`v9` suffix immediately before the
# extension — the pattern found in ~90% of old-marked filenames when this was surveyed, e.g.
# cropping-images-v9.png, Content-Picker2-DataType-v10.png, allow-variance_v10.png,
# start-with-forms-v14.png). The version folder (17/, 18/) already disambiguates, so a leftover
# "-v9" on a freshly-captured current-UI screenshot is just misleading, not meaningful.
#
