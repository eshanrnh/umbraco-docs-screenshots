...
- **Add-on products:** `umbraco-commerce/`, `umbraco-deploy/`, `umbraco-engage/`,
  `umbraco-ui-builder/`, `umbraco-workflow/`, and the other non-CMS areas (
  `umbraco-search/`, `umbraco-automate/`, `ai-*`).

Effective candidate scope: **`$DOCS/<version>/umbraco-cms/**` and `$DOCS/<version>/umbraco-forms/**`** (version = `17` or `18`).
...
`umbraco-cms`/`umbraco-forms` scope), and a Slack request for one should get `❌ Errored: <area>` isn't reproducible
